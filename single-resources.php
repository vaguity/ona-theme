<?php get_header(); ?>
	...
	<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
		...
		<article class="post" id="post-<?php the_ID(); ?>">
			...
			<footer class="meta">
			<div id="post-info">
			Posted <time datetime="<?php echo date(DATE_W3C); ?>" pubdate class="updated"><?php if (function_exists('ap_date')) { ap_date(); echo " &mdash; "; } else { the_date(); echo " &mdash; "; } ?><?php if (function_exists('ap_time')) { ap_time(); } else { the_time('F jS, Y'); } ?></time> in <?php the_category(', '); ?> by <?php the_author(); ?>
